  ...
  try {
    ...
    sendEmail(
      ...
      `<p>Hi ${userName},</p><p>Thank you for registering on our perplexity platform. We're excited to have you on board! Please Verify your email address by clicking the link below : </p>
            <a style="color:green" href="http://localhost:3000/api/auth/verify-email?token=${emailVerificationToken}">Verify Email</a>
...
export async function verifyEmail(req, res) {
  ...
  try {
    ...
    return res.redirect(
      `http://localhost:5173/verification-failed?error=${encodeURIComponent(
        err.message
      )}`
    );
  }
 ...
 * @returns {object} 400 - User not found or error message.
 */
